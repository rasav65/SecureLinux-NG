...
./securelinux-ng.sh --apply --dry-run --config "$TMP_CONFIG" | grep -q 'fstec_items: 11' &&
./securelinux-ng.sh --apply --dry-run --config "$TMP_CONFIG" | grep -q 'fstec_partial: 11' &&
...
./securelinux-ng.sh --check --config "$TMP_CONFIG" | grep -q '2.4' &&
