...
**В проектном реестре 60 позиций, из них 59 реализованы: `done=39`, `partial=20`. Позиция `15.1 kernel.modules_disabled=1` временно не применяется. Поэтому формулировку про «закрываются все 60» считать устаревшей.**
...
| Пакеты дополнительных модулей (chrony, auditd, ufw и др.) | ✗ | Автоматически не удаляются; зависимости password policy, впервые установленные текущим apply и записанные в `installed_packages`, удаляются |
